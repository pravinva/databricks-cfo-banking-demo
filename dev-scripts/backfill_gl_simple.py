#!/usr/bin/env python3
"""
Simple GL backfill script - generates data in Python and inserts in batches.
Avoids complex CROSS JOIN queries that hang the warehouse.
"""
...
print("=" * 80)
print("GL BACKFILL - SIMPLE BATCH INSERT")
print("=" * 80)
...
print("\n1. Generating 24 months × 10 accounts = 240 rows...")
rows = []
...
verify_sql = """
SELECT
    SUBSTRING(account_num, 1, 1) as account_type,
    COUNT(*) as row_count,
...
GROUP BY SUBSTRING(account_num, 1, 1)
...
if result.result and result.result.data_array:
    print(f"\n{'Type':<10} {'Rows':<10} {'Total Credit':<20} {'Total Debit':<20} {'Date Range':<30}")
    print("-" * 90)
    for row in result.result.data_array:
        acct_type = 'Revenue' if row[0] == '4' else 'Expense' if row[0] == '6' else 'Other'
        row_count = int(row[1])
        credit = float(row[2]) if row[2] else 0
        debit = float(row[3]) if row[3] else 0
        date_range = f"{row[4]} to {row[5]}"
        print(f"{acct_type:<10} {row_count:<10} ${credit:<19,.0f} ${debit:<19,.0f} {date_range:<30}")

print("\n" + "=" * 80)
print("COMPLETE")
print("=" * 80)
print("\nNext: Re-run PPNR notebook - should now have realistic revenue/expense data")
