...
fee_income_actual AS (
    ...
    SELECT
        ...
        SUM(CASE
            WHEN account_num LIKE '4100%' THEN credit_amount  -- Service charges
            WHEN account_num LIKE '4110%' THEN credit_amount  -- Card fees
            WHEN account_num LIKE '4120%' THEN credit_amount  -- Loan fees
            WHEN account_num LIKE '4130%' THEN credit_amount  -- Wealth mgmt fees
            ELSE 0
        END) as total_fee_income,

        SUM(CASE WHEN account_num LIKE '4100%' THEN credit_amount ELSE 0 END) as service_charge_income,
        SUM(CASE WHEN account_num LIKE '4110%' THEN credit_amount ELSE 0 END) as card_fee_income,
        SUM(CASE WHEN account_num LIKE '4120%' THEN credit_amount ELSE 0 END) as loan_fee_income,
        SUM(CASE WHEN account_num LIKE '4130%' THEN credit_amount ELSE 0 END) as wealth_fee_income

    FROM cfo_banking_demo.silver_finance.general_ledger
    WHERE transaction_date >= DATE_SUB(CURRENT_DATE(), 730)
    AND account_num LIKE '41%'  -- Revenue accounts
...
expense_actual AS (
    ...
    SELECT
        DATE_TRUNC('month', transaction_date) as month,
        SUM(CASE
            WHEN account_num LIKE '61%' THEN debit_amount  -- Personnel costs
            WHEN account_num LIKE '62%' THEN debit_amount  -- Occupancy
            WHEN account_num LIKE '63%' THEN debit_amount  -- Technology
            WHEN account_num LIKE '64%' THEN debit_amount  -- Marketing
            WHEN account_num LIKE '65%' THEN debit_amount  -- Professional fees
            WHEN account_num LIKE '66%' THEN debit_amount  -- Other operating
            ELSE 0
    ...
    FROM cfo_banking_demo.silver_finance.general_ledger
    WHERE transaction_date >= DATE_SUB(CURRENT_DATE(), 730)
    AND account_num LIKE '6%'  -- Expense accounts
    GROUP BY DATE_TRUNC('month', transaction_date)
)
SELECT
    m.month,
