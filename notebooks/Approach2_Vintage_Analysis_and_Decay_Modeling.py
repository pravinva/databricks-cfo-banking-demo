...
# MAGIC %md
# MAGIC # Approach 2: Vintage Analysis and Component-Based Decay Modeling
# MAGIC
...
WITH base_training AS (
    -- Start with Approach 1 enhanced features
    SELECT
        *,
        DATE_TRUNC('quarter', account_open_date) as cohort_quarter,
        ...
        CASE
            ...
            ELSE 'Expendable'
        END as relationship_category
    FROM cfo_banking_demo.ml_models.deposit_beta_training_data
),
cohort_metrics AS (
...
SELECT
    ...
    -- APPROACH 2: Component Decay Features (Chen)
...
print("✓ Created Approach 2 training dataset with vintage and decay features")

phase2_df = spark.table("cfo_banking_demo.ml_models.deposit_beta_training_phase2")
print(f"\nApproach 2 training records: {phase2_df.count():,}")
...
# MAGIC %md
# MAGIC ### Step 3.2: Train Approach 2 Enhanced Model
...
print(f"Approach 2 Feature Count: {len(all_features)} (vs Approach 1: ~40)")
...
with mlflow.start_run(run_name="approach2_vintage_decay_model") as run:
    ...
    print("=" * 80)
    print("APPROACH 2 MODEL: VINTAGE ANALYSIS + COMPONENT DECAY")
    print("=" * 80)
...
# MAGIC
# MAGIC ### Approach 2 Implementation Complete:
# MAGIC
...
# MAGIC    - 50+ total features (Approach 1 + Approach 2)
...
# MAGIC
# MAGIC ### Next Steps:
# MAGIC - **Approach 3:** Dynamic beta functions for stress testing
