# Treasury Modeling with Databricks (Deposit + PPNR)

This repository contains a focused demo for **Treasury Modeling with Databricks**, centered on:

- **Deposit Modeling** (Approaches 1–3)
- **PPNR / Fee Income Modeling**

## What’s in scope
- **Approach 1**: Static deposit beta model (canonical scoring feature set)
- **Approach 2**: Vintage analysis + component decay runoff forecasting
- **Approach 3**: Dynamic beta + stress testing scenarios
- **PPNR**: Non-interest income (fee) + non-interest expense forecasting → PPNR projections
- **Batch scoring**: Portfolio scoring via `Batch_Inference_Deposit_Beta_Model.py`
- **Reporting**: HTML report generators for executive + regulatory layouts
...
## Repository layout
- `notebooks/`: training, scoring, reporting
- `frontend_app/`: treasury UI
- `backend/`: API + agent integration
- `docs/`: treasury demo script and reporting documentation
...
5. Registration in Unity Catalog (`cfo_banking_demo.models.deposit_beta_model@champion`)
...
3. **Deposit Beta Sensitivity**: Horizontal bar chart colored by beta coefficient
4. **Capital Adequacy Ratios**: Bullet chart with minimum/target reference lines
5. **Liquidity Waterfall**: Waterfall chart showing HQLA sources and cash outflows
6. **Recent Loan Activity**: Table with product type, borrower, date, amount, rate
7. **NIM Components Waterfall**: Waterfall chart showing income/expense breakdown
...
### Dashboard 3: Liquidity Buffer Overview

**Purpose**: High-level liquidity buffer view for LCR context
...
**Data Source**: `cfo_banking_demo.gold_regulatory.lcr_daily`
...
**Data Sources**:
- `cfo_banking_demo.silver_finance.deposit_portfolio`
- `cfo_banking_demo.models.deposit_beta_model` (ML model)
...
**Visualization**: Waterfall chart
- Income components (green): Loan interest, other yield components, fee income
...
│   ├── Approach1_Enhanced_Deposit_Beta_Model.py          # Treasury: Enhanced static beta (XGBoost)
│   ├── Approach2_Vintage_Analysis_and_Decay_Modeling.py  # Treasury: Vintage + runoff forecasting
│   ├── Approach3_Dynamic_Beta_and_Stress_Testing.py      # Treasury: Stress testing (CCAR/DFAST)
...
│                    (Unity Catalog)                              │
├─────────────────────────────────────────────────────────────────┤
│ These tables are created by Treasury modeling notebooks:       │
│                                                                 │
...
│   - Generated by: Approach2_Vintage_Analysis_and_Decay_Modeling.py│
│   - Tracks deposit cohort retention over time                  │
│   - Fields: vintage_quarter, months_aged, survival_rate       │
│                                                                 │
│ • stress_test_results:                                         │
│   - Generated by: Approach3_Dynamic_Beta_and_Stress_Testing.py │
...
│    a. Queries LCR aggregates                                   │
│    b. Applies deposit outflow stress multiplier                │
...
1. **calculate_lcr(deposit_runoff_multiplier)**
   ...
   - Uses: LCR aggregates + deposit outflow stress multiplier
...
### 6. ML Model Training & Analytics Reporting Flow (Approaches 1-3)

The deposit beta modeling and analytics reporting workflow follows a Train → Deploy → Analyze pattern:

```
┌─────────────────────────────────────────────────────────────────┐
│       APPROACH 1: ENHANCED STATIC DEPOSIT BETA MODEL            │
...
│    - Model: cfo_banking_demo.models.deposit_beta_model          │
...
│        "models:/cfo_banking_demo.models.deposit_beta_model@champion" │
│    )                                                            │
│    ↓                                                             │
